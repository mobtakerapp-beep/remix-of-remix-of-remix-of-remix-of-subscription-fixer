...
CREATE POLICY "Service role manages activation codes" ON public.activation_codes FOR ALL TO authenticated USING (false) WITH CHECK (false);
...
CREATE POLICY "Service role manages code redemptions" ON public.code_redemptions FOR ALL TO authenticated USING (false) WITH CHECK (false);
...
CREATE POLICY "Users can manage their own lessons" ON public.user_lessons FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX lesson_shares_token_idx ON public.lesson_shares(token);
...
CREATE POLICY "Users can manage their own shares" ON public.lesson_shares FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE TRIGGER on_auth_user_created_grant_admin AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.grant_admin_by_email();

revoke execute on function public.grant_admin_by_email() from anon, authenticated, public;
